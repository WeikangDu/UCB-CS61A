...
def make_let_frame(bindings, env):
    ...
    def formals(bindings):
        if bindings == nil:
            ...
            check_formals(binding.first)
            return Pair(binding.first, formals(bindings.second))
    def values(bindings, env):
        if bindings == nil:
            return nil
        else:
            binding = bindings.first
            ...
            return Pair(value, values(bindings.second, env))
    return env.make_child_frame(formals(bindings), values(bindings, env))
    # END PROBLEM 15
...
def do_define_macro(expressions, env):
    ...
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        MacroFunc = MacroProcedure(target.second, expressions.second, env)
        env.define(target.first, MacroFunc)
        return target.first
    else:
        bad_target = target.first if isinstance(target, Pair) else target
        raise SchemeError('non-symbol: {0}'.format(bad_target))
