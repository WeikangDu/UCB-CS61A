...
def balanced(m):
    ...
    if is_weight(end(left(m))) and is_weight(end(right(m))):
        ...
        return length(left(m))*size(end(left(m))) == length(right(m))*total_weight(end(right(m))) and balanced(end(right(m)))
    elif is_weight(end(left(m))):
        return length(right(m))*size(end(right(m))) == length(left(m))*total_weight(end(left(m))) and balanced(end(left(m)))
    else:
...
def totals_tree(m):
    ...
    def new_tree(m):
        if is_weight(m):
            ...
            root = total_weight(m)
            branches = [new_tree(end(left(m))), new_tree(end(right(m)))]
...
def make_joint(withdraw, old_password, new_password):
    ...
    if type(error) == str:
        return error
    def joint(amount, try_pass):
        if try_pass == new_password:
...
def generate_paths(t, x):
    ...
    if label(t) == x:
        yield [x]
    for b in branches(t):
        iterator_over_rests_of_path = generate_paths(b, x)
        for rest_of_path in iterator_over_rests_of_path:
            yield [label(t)] + rest_of_path
###################
# Extra Questions #
###################
